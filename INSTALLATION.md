# Install guide

## Installing the plugin
1. Download `Angelscript-<version>.zip` from [releases](https://github.com/aesirinteractive/rider-unreal-angelscript/releases)
2. In Rider, open `Settings > Plugins > ⚙️ > Install Plugin from Disk` or run action `Install Plugin from Disk`
3. Select the downloaded `Angelscript-<version>.zip`

## Installing the syntax highlighting
1. Download `angelscript-syntax.zip` from [releases](https://github.com/aesirinteractive/rider-unreal-angelscript/releases) and extract it somewhere
2. In Rider, open settings and search for `TextMate Bundles`
3. Click the `+` icon and select the extracted `angelscript-syntax` folder
